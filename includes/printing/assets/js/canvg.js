	...
	function build(opts) {
		...
		svg.parseXml = function(xml) {
			if (
				...
				// try catch was added.
				try {
					// added by wang jia (start)
					xml = xml.replace(' xmlns:NS1=""', '');
					xml = xml.replace(' NS1:xmlns:xlink="http://www.w3.org/1999/xlink"', '');
					xml = xml.replace(' xmlns="http://www.w3.org/2000/svg"', '');

					var svgXml = jQuery.parseXML(xml);
					var svgObj = jQuery(svgXml).find('svg');
					svgObj.removeAttr('xmlns:xlink').removeAttr('viewBox');
					xml = jQuery('<div>')
						.append(svgObj.clone())
						.html();
					// (end)
		...
		svg.Property.prototype.addOpacity = function(opacityProp) {
			...
			if (opacityProp.value != null && opacityProp.value != '' && typeof this.value == 'string') {
				...
				if (color.ok) {
					newValue =
						'rgba(' + color.r + ', ' + color.g + ', ' + color.b + ', ' + opacityProp.numValue() + ')';
				}
			}
		...
		svg.Element.text = function(node) {
			...
			this.getBoundingBox = function() {
				...
				return new svg.BoundingBox(
					x,
					y - fontSize,
					x + Math.floor((fontSize * 2.0) / 3.0) * this.children[0].getText().length,
					y
				);
			};
		...
		svg.loadXmlDoc = function(ctx, dom) {
			...
			if (svg.opts['ignoreMouse'] != true) {
				ctx.canvas.onclick = function(e) {
					var p = mapXY(
						new svg.Point(e != null ? e.clientX : event.clientX, e != null ? e.clientY : event.clientY)
					);
